...
export Solve_eq_float2D,simulate_eq_2D, Solve_eq_peg2D
...
global y_t = 1.0
global θ = 0.0385
global δ_1 = 0.25 #-0.35
global δ_2 = 2.37 # 0.46
global ρ = 0.93
global σ_μ = 0.037
global r =0.01
global n_y = 20
...
function Solve_eq_float2D(T)
    ...
    for t in T-1:-1:1
        ...
        Threads.@threads for j in 1:n_y #@Threads.@threads
            ...
            for id in 1:n_d
                val_d = 0.0
                for jj in n_y
                    val_d = copy(val_d)+ P_stat[j,jj]*β*(θ*V_f_spline[t+1,jj ](0.0,0.0)+ (1-θ)*v_d[t+1, jj,1])         
                end
                c_t_d = y_t_stat[j] -grid_d[id]
                c_n_d =1.0
                v_d[t, j,id] = utilityCRRA(final_good(max(y_t_stat[j] -grid_d[id],1e-6), c_n_d))  - L(y_t_stat[j])+ val_d
                for i in 1:n_B
                    if(t==T-1 )
                        x_0[:,j] = [0.0,0.001]
                    else
        ...
        println(sum(d[t,:,:,:]))
        if(maximum(abs.(v_f[t,:,:,:].-v_f[t+1,:,:,:]))+maximum(abs.(q[t,:,:,:].-q[t+1,:,:,:])) <=1e-3)
            flag =0.0
        end
    ...
    #zero_ind = searchsortedfirst(grid_B)
    for j in 1:n_y 
        for id in 1:n_d
            if d[1,j,1, id]==1.0
                default_border[j, id] = 0.0
            elseif(v_f[1,j,n_B, id]<v_d[1, j, id]) && (v_f[1,j,1, id]>v_d[1, j, id])
                default_border[j, id] = find_zero(x->V_f_spline[1,j](x, grid_d[id])- v_d[1, j, id], (grid_B[1],grid_B[n_B] ),Roots.Brent() )
            else
...
function Solve_eq_peg2D(T)
    ...
    for t in T-1:-1:1
        ...
        Threads.@threads for j in 1:n_y #@Threads.@threads
            ...
            for id in 1:n_d
                ...
                for i in 1:n_B
                    ...
                    func(x::Vector, grad::Vector) = deepcopy(interp_alg_const(copy(V_f_spline[t+1,:]), q_spline[t,j], j ,grid_B[i],grid_d[id],x))
                    #func(x::Vector) = deepcopy(interp_alg_unconst(copy(V_f_spline[t+1,:]), q_spline[t,j], j ,grid_B[i],grid_d[id],x))
                    max_func[j] = func
                    #constraint(x::Vector, grad::Vector) = y_t_stat[j]-grid_B[i]-grid_d[id]+ q_spline[j](x[1],x[2])*x[1] +q_d(x[2])*x[2]- c_t_min 
                    #constraint(x::Vector, grad::Vector) = x[1] - grid_B[n_B]
        ...
        println(maximum(abs.(q[t,:,:,:].-q[t+1,:,:,:])))
        println(sum(d[t,:,:,:]))
        if(maximum(abs.(v_f[t,:,:,:].-v_f[t+1,:,:,:]))+maximum(abs.(q[t,:,:,:].-q[t+1,:,:,:])) <=5e-2)
            flag =0.0
        end
        if(maximum(abs.(v_f[t,:,:,:].-v_f[t+1,:,:,:]))<=1e-5)
            v_f[1,:,:,:] = v_f[t,:,:,:]
...
function simulate_eq_2D( Model_solution; burnout = 10000, t_sim =1010000, n_sim = 10, ERR="float", method = "spline_sch")
    global P_stat, y_t_stat = DiscretizeAR(ρ, σ_μ, n_y, "tauchen")
        ...
        for t in 2:t_sim
           ...
           if(Default_flag==1.0) 
                ...
                if(Default_flag==1.0)
                    ...
                    Y_t[i,t] = y_t_stat[SIM[i,t]]
                    B_hist[i,t] = max(policy_B_spline[SIM[i,t]](0.0, 0.0), grid_B[1])
                    d_hist[i,t] = max(policy_d_spline[SIM[i,t]](0.0,0.0),grid_B[1])
                    C_t[i,t] = Y_t[i,t] + q_spline[SIM[i,t]](0.0, 0.0)*B_hist[i,t] + q_d(d_hist[i,t])*d_hist[i,t]
                    if C_t[i,t] < c_t_min && ERR =="peg"
                        C_n[i,t] =  max(min(copy(C_t[i,t])^(power)*const_w,1.0),1e-6)
                        h_t[i,t] = C_n[i,t]^(1.0/α)
                    else
                    ...
                    C[i,t] = final_good(C_t[i,t], C_n[i,t]) 
                    R[i,t] = 1.0/q_spline[SIM[i,t]](0.0,0.0) - 1.0 
                    D[i,t] = 0.0
                    D_state[i,t] = 0.0
                    Trade_B[i,t] = (Y_t[i,t] - C_t[i,t])/Y_t[i,t]
                    Default_flag = 0.0
                ...
                if Def_bord_itp[SIM[i,t]](B_hist[i,t-1],d_hist[i,t-1] ) <1.0  #case of default
                    ...
                    C[i,t] = final_good(C_t[i,t], C_n[i,t]) 
                    R[i,t] = 1.0/q_spline[SIM[i,t]](B_hist[i,t-1], d_hist[i,t-1]) - 1.0
                    D[i,t] = 0.0
                    D_state[i,t] = 0.0
                    Trade_B[i,t] = (Y_t[i,t] - C_t[i,t])/Y_t[i,t]
                    Default_flag = 0.0
   ...
   #calibration targets 
    ...
    writedlm( "output/results_pre_def_2D.csv",  stats, ',')  
    ...
    p6 = plot(-15.0:1.0:14.0,stats[10,60:89], ylabel = "d", xlabel = "T")
    
    plot(p1,p2, p3,p4,p5, p6, layout = (3, 2))
    png("pre_def_stats_2D")
